...
마이그레이션 기간에는 기존 `frouter` 설치와 설정도 계속 지원합니다.
패키지는 `frouter` 명령 별칭을 함께 제공하고, 첫 실행 시 `~/.frouter.json`을
`~/.free-router.json`으로 가져오며, 새 `FREE_ROUTER_*` 환경 변수 이름이 없으면
기존 `FROUTER_*` 환경 변수를 fallback으로 사용합니다.
