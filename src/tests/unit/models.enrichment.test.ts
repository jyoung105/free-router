...
test("each hardcoded NIM model has required fields", async () => {
  await withCleanEnv(async () => {
    ...
    assert.ok(
      models.length > 50,
      `Expected 50+ NIM models, got ${models.length}`,
    );
    ...
    for (const m of models) {
      ...
      assert.ok(
        ...
        `Model ${m.id} missing displayName`,
      );
      assert.ok(
        typeof m.context === "number" && m.context > 0,
        `Model ${m.id} missing context`,
      );
      assert.equal(
        m.providerKey,
        "nvidia",
        `Model ${m.id} has wrong providerKey`,
      );
      assert.ok(Array.isArray(m.pings), `Model ${m.id} missing pings array`);
      assert.equal(
        m.status,
        "pending",
        `Model ${m.id} should start as pending`,
      );
      assert.equal(
        m.httpCode,
        null,
        `Model ${m.id} should start with null httpCode`,
      );
    }
  });
});
...
test("models are enriched with tier information from rankings", async () => {
  await withCleanEnv(async () => {
    ...
    assert.ok(
      withTier.length > 10,
      `Expected 10+ models with tier info, got ${withTier.length}`,
    );
  });
});
...
test("models may have sweScore and aaIntelligence from rankings", async () => {
  await withCleanEnv(async () => {
    ...
    for (const m of withIntel) {
      assert.ok(
        typeof m.aaIntelligence === "number",
        `aaIntelligence should be number for ${m.id}`,
      );
    }
  });
});
...
test("models with sweScore get tier based on score thresholds", async () => {
  await withCleanEnv(async () => {
    ...
    for (const m of withSwe) {
      assert.ok(
        typeof m.sweScore === "number",
        `sweScore should be number for ${m.id}`,
      );
      assert.ok(
...
test("hardcoded NIM list includes key models from each tier", async () => {
  await withCleanEnv(async () => {
    ...
    // A tier
    assert.ok(
      ids.has("meta/llama-3.1-405b-instruct"),
    ...
    // C tier
    assert.ok(
      ids.has("mistralai/mixtral-8x7b-instruct-v0.1"),
      "Missing Mixtral 8x7B",
    );
  });
});
...
test("hardcoded NIM list has no duplicate model IDs", async () => {
  await withCleanEnv(async () => {
    ...
    assert.equal(
      ids.length,
      uniqueIds.size,
      `Found ${ids.length - uniqueIds.size} duplicate model IDs`,
    );
  });
});
