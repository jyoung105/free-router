...
async function withMockedHttps(
  responder: (opts: any) => MockOutcome,
  run: () => Promise<void>,
) {
...
test("getAllModels happy path: uses bundled NVIDIA list when fetch is disabled", async () => {
  await withCleanApiKeyEnv(async () => {
    await withNoFetchEnv("1", async () => {
      ...
      assert.ok(
        models.length > 20,
        `expected bundled NIM list, got ${models.length}`,
      );
      assert.equal(
        models.some((m) => m.providerKey === "openrouter"),
        false,
      );
  ...
  await withCleanApiKeyEnv(async () => {
    await withNoFetchEnv(undefined, async () => {
      await withMockedHttps(
        (opts) => {
          if (
            opts.hostname === "integrate.api.nvidia.com" &&
            opts.path === "/v1/models"
          ) {
            return {
              type: "response",
          ...
          return {
            type: "error",
            error: new Error(
              `unexpected request: ${opts.hostname}${opts.path}`,
            ),
          };
        },
        async () => {
          ...
          assert.equal(
            openrouter.some((m) => /:paid$/.test(m.id)),
            false,
          );
          assert.ok(
            openrouter.every(
              (m) => m.status === "pending" && Array.isArray(m.pings),
            ),
          );
        },
      );
    ...
    await withNoFetchEnv(undefined, async () => {
      await withMockedHttps(
        ...
        async () => {
          ...
          assert.ok(
            models.length > 20,
            "expected hardcoded NVIDIA fallback list",
          );
          assert.equal(
            models.some((m) => m.providerKey === "openrouter"),
            false,
